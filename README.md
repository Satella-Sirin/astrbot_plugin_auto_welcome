...
- 专属消息：
  ...
  87654321:欢迎 {at} 来到B群！
- 分段符号：`---`
- 白名单群：`12345678`、`87654321`、`99999999`
...
- 在群 `87654321` 中，机器人发送：
  欢迎 @新成员 来到B群！
- 在群 `99999999` 中（无专属消息），机器人发送全局默认消息：
  欢迎 @新成员 加入本群！
  请阅读群规。

若您还设置了分段符号 `---`，则欢迎消息会按 `---` 拆分成多条发送。
...
- 初始版本发布
- 支持白名单群和自定义欢迎消息
- 支持 `{nickname}` 和 `{at}` 两个占位符
- 支持为不同群设置专属欢迎词（多行文本配置）
- 实现自定义分段符号，灵活控制消息发送方式
- 集成 WebUI 配置界面
- 采用 MIT 许可证
- 明确仅支持 OneBot 协议
- 优化代码结构，拆分配置解析方法，增加空配置警告
